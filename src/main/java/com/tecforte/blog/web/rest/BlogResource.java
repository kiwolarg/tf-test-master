...
public class BlogResource {
    ...
    /**
     * {@code DELETE  /blogs/clean} : delete the "certain content" of blog.
     *
     * @param keywords the keywords of the entryDTO to clean.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/blogs/clean")
    public ResponseEntity<Void> cleanEntries(String[] keywords) {
    	
    	if(keywords != null) {
    		entryService.findAllToClean(keywords);
    	}
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, keywords.toString())).build();
    }
    
    /**
     * {@code DELETE  /blogs/{id}/clean} : delete the "certain content" of blog.
     *
     * @param id of the entryDTO parent, keywords the keywords of the entryDTO to clean.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/blogs/{id}/clean")
    public ResponseEntity<Void> cleanEntries(@PathVariable Long id, String[] keywords) {
    	
    	if(keywords != null) {
    		entryService.findAllByBlogIdAndKeywordsToClean(id, keywords);
    	}
